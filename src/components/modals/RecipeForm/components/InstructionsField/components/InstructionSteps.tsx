  ...
  return (
    <div className={css.container}>
      <div className={css.instructionHeader}>
        {props.instruction}
        <Button kind="ghost" onClick={props.onDeleteInstruction}>удалить</Button>
      </div>
      <div className={css.steps}>
        {fields.map((field, index) => (
          <div className={css.stepContainer} key={index}>
            ...
            <Button kind="ghost" onClick={() => remove(index)}>удалить</Button>
          </div>
        ))}
        <Button kind="ghost" className={css.addStepButton} onClick={addStep}>Добавить шаг</Button>
      </div>
    </div>
  );
};
